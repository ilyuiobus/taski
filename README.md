...
Для локальной работы с SQLite вместо PostgreSQL можно указать:

```dotenv
USE_SQLITE=true
```

В этом случае Django использует SQLite вместо PostgreSQL.
...
| `USE_SQLITE` | переключение между SQLite и PostgreSQL |
...
11. отправляет сообщение об успешном деплое в Telegram.
...
В production используются Docker-образы из Docker Hub.
